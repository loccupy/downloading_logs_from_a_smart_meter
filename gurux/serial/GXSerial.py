...
import threading
import traceback
import gurux.common.io.BaudRate
import gurux.common.io.Parity
import gurux.common.io.StopBits
from gurux.common.enums import TraceLevel, MediaState, TraceTypes
from gurux.common.IGXMedia import IGXMedia
from gurux.common.MediaStateEventArgs import MediaStateEventArgs
from gurux.common.TraceEventArgs import TraceEventArgs
from gurux.common.PropertyChangedEventArgs import PropertyChangedEventArgs
from gurux.common.ReceiveEventArgs import ReceiveEventArgs
...
class GXSerial(IGXMedia):
    def __init__(self,
                 port,
                 baudRate=gurux.common.io.BaudRate.BAUD_RATE_9600,
                 dataBits=8,
                 parity=gurux.common.io.Parity.NONE,
                 stopBits=gurux.common.io.StopBits.ONE):
        """Constructor.
        port : Serial port.
        baudRate : Baud rate.
        dataBits : Data bits.
    ...
    def getAvailableBaudRates(self):
        ...
        return (gurux.common.io.BaudRate.BAUD_RATE_300, gurux.common.io.BaudRate.BAUD_RATE_600,
                gurux.common.io.BaudRate.BAUD_RATE_1800, gurux.common.io.BaudRate.BAUD_RATE_2400,
                gurux.common.io.BaudRate.BAUD_RATE_4800, gurux.common.io.BaudRate.BAUD_RATE_9600,
                gurux.common.io.BaudRate.BAUD_RATE_19200, gurux.common.io.BaudRate.BAUD_RATE_38400)
    ...
    def __readThread(self):
        ...
        while not self.__closing.is_set():
            try:
                data = self.__h.read()
                if data is not None:
    ...
    def open(self):
        ...
        if self.__trace & TraceLevel.INFO != 0:
            ...
            self.__notifyTrace(TraceEventArgs(TraceTypes.INFO, \
                                              "Settings: Port: " + self.__portName + " Baud Rate: " + str(
                                                  self.baudRate) + \
                                              " Data Bits: " + str(int(self.dataBits)) + " Parity: " + str(
                                                  self.parity) + \
    ...
    def __getBaudRate(self):
        if self.__h.isOpen():
            self.__h.baudRate = gurux.common.io.BaudRate(self.__h.getBaudRate())
    ...
    def __getParity(self):
        if self.__h.isOpen():
            self.__h.parity = gurux.common.io.Parity(self.__h.getParity())
    ...
    def __getStopBits(self):
        if self.__h.isOpen():
            self.__h.stopBits = gurux.common.io.StopBits(self.__h.getStopBits())
    ...
    def getSettings(self):
        ...
        if self.__h.baudRate != gurux.common.io.BaudRate.BAUD_RATE_9600:
            sb += "<BaudRate>"
            sb += str(int(self.__h.baudRate))
            sb += "</BaudRate>"
            sb += nl

        if self.__h.stopBits != gurux.common.io.StopBits.ONE:
            sb += "<StopBits>"
            sb += str(int(self.__h.stopBits))
            sb += "</StopBits>"
            sb += nl

        if self.__h.parity != gurux.common.io.Parity.NONE:
            sb += "<Parity>"
    ...
    def setSettings(self, value):
        ...
        self.__portName = ""
        self.__h.baudRate = gurux.common.io.BaudRate.BAUD_RATE_9600
        self.__h.stopBits = gurux.common.io.StopBits.ONE
        self.__h.parity = gurux.common.io.Parity.NONE
        self.__h.dataBits = 8
