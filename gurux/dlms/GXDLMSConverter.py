...
class GXDLMSConverter:
    ...
    def __getObjects(cls, standard):
        ...
        if standard == Standard.ITALY:
            str_ = importlib.resources.read_binary(__name__, "Italy.txt").decode("utf-8")
        elif standard == Standard.INDIA:
            str_ = importlib.resources.read_binary(__name__, "India.txt").decode("utf-8")
        elif standard == Standard.SAUDI_ARABIA:
            str_ = importlib.resources.read_binary(__name__, "SaudiArabia.txt").decode("utf-8")
        elif standard == Standard.SPAIN:
            str_ = importlib.resources.read_binary(__name__, "Spain.txt").decode("utf-8")
        if not str_:
            return None
        str_ = str_.replace("\n", "\r")
        rows = str_.split('\r')
    ...
    def __readStandardObisInfo(cls, standard, codes):
        ...
        str_ = importlib.resources.read_binary(__name__, "OBISCodes.txt").decode("utf-8")
        str_ = str_.replace("\n", "\r")
        rows = str_.split('\r')
        for it in rows:
            if it and not it.startswith("#"):
