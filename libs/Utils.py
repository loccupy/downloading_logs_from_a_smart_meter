...
EventSelfDiagCode = [
    ...
    (133, "Переполнение буфера при заполнении тела пуш сообщения"),
    (134, "Сброс отправки пуша по TCP по модему"),
    (135, "Прекращение отправки пуша"),
    (136, "Критическое исключение"),
    (137, "Ошибка источника тактирования процессора"),
    (138, "Несоответствие состояния реле"),
    (139, "Переполнения стека")
]
...
def create_sheet_in_excel_file(data, writer, sheet_name, config, check_sample, attempt=1, max_attempts=3):
    ...
    try:
        ...
        if attempt < max_attempts:
            ...
            message_in_out(f'Обрыв соединения при выгрузке журнала {sheet_name}')
            return None
